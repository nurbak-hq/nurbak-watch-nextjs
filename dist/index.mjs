...
var eventQueue, flushTimer, config, nextFlushAllowedAt;
...
function installIncomingApiInterceptor(cfg) {
  ...
  const unpatchHttp = patchServerPrototype(http.Server.prototype);
  const unpatchHttps = patchServerPrototype(https.Server.prototype);
  unpatchHttpServers = () => {
    unpatchHttps();
    unpatchHttp();
